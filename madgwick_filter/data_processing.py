...
from madgwick_filter import utils

def import_data(filename, t_last=False):
    '''
    Importing t, x, y, z data from 0, 2, 3, 4 columns of the .csv file 

    return: data_t - array of timestamps
            data_xyz - array of x, y, z components for each timestamp 
    '''
    data_np = pd.read_csv(filename, header=1).to_numpy()
    data_t = data_np[:, 0]
    if(t_last): data_t = data_np[:, data_np.shape[1]-1]
    data_xyz = np.empty((len(data_t), 3))
    data_xyz[:, 0] = data_np[:, 2]
    data_xyz[:, 1] = data_np[:, 3]
    data_xyz[:, 2] = data_np[:, 4]

    return data_t, data_xyz
...
def import_realsense_data(filename):
    '''
    Importing realsense data from .txt file with order: time, rotation quaternion, translation

    return: data_quat_t, data_quat_R, data_quat_T
    '''
    data_quat_np = pd.read_csv(filename, header=0, delimiter=" ").dropna().to_numpy()
...
def unbias_quat_array(sm_data, mocap_data, i_shift):
    '''
    Deprecated
    Unbiases quaternions array by taking sm_data starting from i_shift
    and multiplying both arrays by corresponding inverse to the first 
    quaternion in array - to make it start from identity quaternion
    '''
    sss = sm_data[i_shift:].copy()
    mmm = mocap_data[:-i_shift].copy()

    anti_mmm0 = mmm[0] * np.array([1, -1, -1, -1])
    delta_quat_mmm0 = utils.quaternion_multiply(sss[0], anti_mmm0)

    mmm_unbiased = np.array([utils.quaternion_multiply(delta_quat_mmm0, mmm[i]) for i in range(len(mmm))])

    return sss, mmm_unbiased
